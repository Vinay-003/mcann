...
def main():
    ...
    x_stream = np.arange(72)
    x_ground_truth = np.arange(72, 72+72)
    x_predicted = np.arange(72, 72+72)
                        
    plt.plot(x_stream, stream_data[-72:], label='Stream Data', color='black')
    ...
    max_value = max(max(ground_truth), max(predicted), max(stream_data[-72:]))
